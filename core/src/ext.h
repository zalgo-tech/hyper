...
char* inpf(int max, char* src) {
	...
	fp = fopen(src,"r+");
	if (fp == NULL) {
		printf("ERROR : File or input stream could not be found.\n");
		return "";
	}
	while (fgets(next,2,fp) != NULL) {
		if (strcmp(next,"\n") == 0) {
			return str;
		}
		if (size > max and max > 0) {
			return str;
		}
		strcat(str,next);
		size++;
...
struct list {
	...
	T get(int idx) {
		...
		dlnode<T>* current;
		bool forward = true;
		if (idx < end / 2) { current = first; }
		else { current = last; forward = false; }
		for (int i = 0; i < idx; i++) {
			if (forward) { current = current->next; }
